function [RespMatrix,RespMatrixTraining,RespMatrixTesting,RespMatrixAllTrials,ProbDist,BinThresh,removedBlocks_new,selected_cells] = get_prob_dist_ver8_for20_pooled_ST(ExpID,selected_cells,bins,folds,test_set)
...
%   Detailed explanation goes here
    ...
    current_dir = pwd;
    cd ../..
    load('dataOut_NatScenes_POOLED2.mat');
    cd(current_dir)
    ...
    removedBlocks_all = squeeze(dataOut.isRemovedBlock(:,:,:)); % FOR MIXTURE
    removedBlocks_sum = squeeze(sum(removedBlocks_all,1));
    removedBlocks_max = max(removedBlocks_sum,[],1);
    ...
    for c = 1:length(selected_cells)
        ...
        for stim = 1:dataOut.totalNumStimuli %for each stim
            ...
            removedBlocks_n = squeeze(dataOut.isRemovedBlock(:,cell_N,stim));
            count=0;
            while sum(removedBlocks_n)<removedBlocks_max(stim)
        ...
        %leng = length(cell_N_resps_sorted);        
        max5_resp = cell_N_resps_sorted(end-4); %get the fifth highest value
        
        
        %get bin thresholds for this cell
        cell_N_bin_thresh = zeros(1,bins-1);
        for b = 1:bins-1
            cell_N_bin_thresh(b) = (max5_resp/bins)*b;
        end
